...
export function HomeView({ onNavigate }: { onNavigate: (view: MainView) => void }) {
  ...
  return <section className="home-view" aria-labelledby="home-title">
    ...
    <div className="home-primary-grid">
      <button type="button" className="home-create-card" onClick={() => onNavigate('agenda')}><span className="action-icon"><CalendarDays size={30} aria-hidden="true" /></span><span className="home-action-copy"><strong>Agenda</strong><span>Consultá las actividades y organizá tu día.</span></span><span className="home-card-bottom"><span>Calendario o lista</span><ArrowRight size={24} aria-hidden="true" /></span></button>
      <button type="button" className="home-agenda-card" onClick={() => onNavigate('technical')}><span className="action-icon"><Settings2 size={30} aria-hidden="true" /></span><span className="home-action-copy"><strong>Datos técnicos de sala</strong><span>Elegí un espacio y consultá su ficha completa.</span></span><span className="home-card-bottom"><span>Capacidad, escenario y equipamiento</span><ArrowRight size={24} aria-hidden="true" /></span></button>
    </div>
