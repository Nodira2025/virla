...
export type MainView = 'home' | 'agenda' | 'reserve' | 'store' | 'spaces' | 'technical' | 'request' | 'requests' | 'admin';
export const VIEW_HASHES: Record<MainView, string> = { home: '#inicio', agenda: '#agenda', reserve: '#crear-agenda', store: '#tienda-virla', spaces: '#espacios', technical: '#datos-tecnicos', request: '#propuesta-externa', requests: '#solicitudes', admin: '#admin' };
...
export function AppShell({ activeView, onNavigate, children }: { activeView: MainView; onNavigate: (view: MainView) => void; children: ReactNode }) {
  ...
  return (
    <div className="virla-app">
      ...
      {menuOpen && <div className="mobile-more" id="mobile-more"><div className="flex items-center justify-between"><strong>Más en Virla</strong><button type="button" onClick={() => setMenuOpen(false)} aria-label="Cerrar menú"><X size={22} /></button></div>{visibleNavigation.filter((item) => ['requests', 'admin', 'request', 'store'].includes(item.view)).map(({ view, label, icon: Icon }) => <a href={VIEW_HASHES[view]} key={view} onClick={(event) => { event.preventDefault(); navigate(view); }}><Icon size={22} aria-hidden="true" />{label}<ChevronRight size={18} aria-hidden="true" /></a>)}<a href="tel:+543814221692"><Phone size={22} aria-hidden="true" />Contactar al Virla</a></div>}
